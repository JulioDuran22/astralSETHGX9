...
let img = 'https://qu.ax/YDrVR.jpg';
let time = user.lastmiming + 600000;

if (new Date() - user.lastmiming < 600000) {
return conn.reply(m.chat, `${emoji3} Debes esperar ${msToTime(time - new Date())} para volver a minar.`, m, rcanal);
}
...
await conn.sendFile(m.chat, img, 'nino.jpg', info, fkontak);
await m.react('⛏️');
